...
enum State {
	CW00, CW01, CW10, CW11, CCW00, CCW01, CCW10, CCW11
};
...
void init_intc(void) {
	...
	XIntc_Connect(&sys_intc,
			...
			(XInterruptHandler) btnint_handler, &btn_gpio);
	XIntc_Connect(&sys_intc,
			XPAR_MICROBLAZE_0_AXI_INTC_ENCODER_IP2INTC_IRPT_INTR,
			(XInterruptHandler) encoderint_handler, &encoder_gpio);
	...
	XIntc_Enable(&sys_intc,
			XPAR_MICROBLAZE_0_AXI_INTC_AXI_GPIO_BTN_IP2INTC_IRPT_INTR);
	xil_printf("btn interrupt initialized!\n\r");
	XIntc_Enable(&sys_intc,
			XPAR_MICROBLAZE_0_AXI_INTC_ENCODER_IP2INTC_IRPT_INTR);
...
void init_encodergpio(void) {
	/* Initialize the GPIO driver */
	XGpio_Initialize(&encoder_gpio, XPAR_ENCODER_DEVICE_ID);
	/* This function ensures interrupts are configured for this device then enables the interrupt. */
...
void init_lcdspi(void) {
	...
	int status = XSpi_CfgInitialize(&lcd_spi, configPtr,
...
/*
 ...
 QActive_postISR((QActive *)&AO_final, DEFAULT);
...
void btnint_handler() {
	...
	switch (btn_value) {
	case BTNU:
		QActive_postISR((QActive *) &AO_final, BUTTON_UP);
		//xil_printf("btn_up");
		break;
	case BTNL:
		QActive_postISR((QActive *) &AO_final, BUTTON_LEFT);
		break;
	case BTNR:
		QActive_postISR((QActive *) &AO_final, BUTTON_RIGHT);
		break;
	case BTND:
		QActive_postISR((QActive *) &AO_final, BUTTON_DOWN);
		break;
	case BTNC:
		QActive_postISR((QActive *) &AO_final, BUTTON_CENTER);
		break;
	}
...
void encoderint_handler(void) {
	...
	if (encoder & 0b100) {
		QActive_postISR((QActive *) &AO_final, ENCODER_CLICK);
	}
	...
	case 0b01000:
		state = CW00;
		QActive_postISR((QActive *) &AO_final, ENCODER_DOWN);
		break;
	case 0b00010:
	case 0b10010:
		state = CCW10;
	...
	case 0b10100:
		state = CCW00;
		QActive_postISR((QActive *) &AO_final, ENCODER_UP);
		break;
	}
